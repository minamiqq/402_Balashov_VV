...
class StudentsList implements Iterator
{
    ...
    public function __construct(FileLogger $fileLogger, DBLogger $dbLogger)
    {
        $this -> fileLogger = $fileLogger;
        $this -> dbLogger = $dbLogger;
        $this -> fileLogger -> log(date("d.m.Y"), date("H:i:s"), "Cоздан новый список студентов");
        $this -> dbLogger -> log(date("d.m.Y"), date("H:i:s"), "Cоздан новый список студентов");
    }

    public function add(Student $student)
    {
        array_push($this -> students, $student);
        $this -> fileLogger -> log(date("d.m.Y"), date("H:i:s"), "В список добавлен новый студент");
        $this -> dbLogger -> log(date("d.m.Y"), date("H:i:s"), "В список добавлен новый студент");
    }
